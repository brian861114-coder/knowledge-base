...
def is_ignored_note_path(path: Path, vault_root: Path) -> bool:
    try:
        ...
        parts = path.parts
    return any(part.startswith("_bak") or part.startswith(".trash") for part in parts)
...
def load_local_config(config_path: Path | None = None) -> dict:
    ...
    for key in ("pythonPath", "python_path", "vaultPath", "vault_path", "outDir", "out_dir"):
        value = data.get(key)
...
def resolve_vault_path(cli_value: str | None = None, config_path: Path | None = None) -> Path:
    ...
    raise FileNotFoundError(
        "Vault path not configured. Pass --vault, set KB_VAULT_PATH, or define vaultPath/vault_path in .knowledge-base.local.json."
    )
