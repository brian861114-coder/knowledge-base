...
def validate_note(path: Path, vault: Path, schema: dict) -> list[str]:
    ...
    normalized_headings, legacy_hits = normalize_sections(raw_sections, rename_map)
    headings_set = set(normalized_headings)
    ...
    for issue in check_required_order(normalized_headings, required_order):
        issues.append(f"{rel_path}: {issue}")

    for issue in check_conditional_requirements(headings_set, body, conditional_requirements):
        issues.append(f"{rel_path}: {issue}")

    for issue in check_map_optional_positions(normalized_headings, ordering_rules):
        issues.append(f"{rel_path}: {issue}")
