...
const TAXONOMY_LABELS = {
  mechanics: "?õÂ≠∏",
  electromagnetism: "?ªÁ?Â≠?,
  waves_optics: "Ê≥¢Â??áÂ?Â≠?,
  foundations: "?∫Á?Á∏ΩË?",
  thermo_fluids: "?±Â≠∏?áÊ?È´?,
  modern_physics: "Ëøë‰ª£?©Á?",
  analytical_dynamics: "Ëß???ïÂ?Â≠?,
  uncategorized: "?™Â?È°?,
};
...
const TYPE_LABELS = {
  ...
  concept: "Ê¶ÇÂøµ",
  quantity: "?©Á???,
  mathematical_tool: "?∏Â≠∏Â∑•ÂÖ∑",
  experiment: "ÂØ¶È?",
};

const RELATION_LABELS = {
  requires: "?àÂ??ú‰?",
  derives_to: "?ØÊé®Â∞éÂá∫",
  formalized_by: "?∏Â≠∏?ØÊ?",
  related_to: "?∏È?Ê¶ÇÂøµ",
  organized_by: "‰∏ªÈ??∂Á?",
  verified_by: "È©óË?ÂØ¶È?",
  measures: "?èÊ∏¨Â∞çÊ?",
  uses: "?¥Êé•‰ΩøÁî®",
  explains: "Âª∂‰º∏Ë¶ñË?",
};
...
init().catch((error) => {
  console.error(error);
  els.detailTitle.textContent = "ËºâÂÖ•Â§±Ê?";
...
async function fetchJson(url) {
  ...
  if (!response.ok) {
    throw new Error(`${url} ËºâÂÖ•Â§±Ê?ÔºöHTTP ${response.status}`);
  }
...
function normalizeGraph(rawGraph) {
  ...
  const domainHubs = TAXONOMY_ORDER.map((taxonomy) => ({
    ...
    summary: `${TAXONOMY_LABELS[taxonomy]} ?ÑÁ∏ΩË¶ΩÊ?Á¥ê„ÄÇ`,
    type: "domain",
    taxonomy,
    tier: 0,
    degree: 0,
  ...
  const overviewRoot = {
    ...
    title: "?©Á??•Ë??ñË?",
    shortTitle: "?©Á??•Ë??ñË?",
    summary: "ÂæûÈ??üÁ∏ΩË¶ΩÈÄ≤ÂÖ•Ê¶ÇÂøµ?ÅÂ?ÂæãË??®Â??ú‰???,
    type: "root",
    taxonomy: "all",
    tier: 0,
    ...
    domain: "",
    tags: [],
    searchText: "?©Á? ?•Ë??ñË? Á∏ΩË¶Ω physics knowledge map",
  };
...
function renderRings(nodes) {
  ...
  els.ringLayer.innerHTML = [
    ...
    `<text class="ring-label" x="${focal.x}" y="${focal.y - 258}" text-anchor="middle">?àÂ?</text>`,
    `<text class="ring-label" x="${focal.x}" y="${focal.y + 440}" text-anchor="middle">Âª∂‰º∏</text>`,
  ].join("");
}
...
function renderNodes(nodes) {
  ...
  const markup = nodes
    .map((node) => {
      ...
      return `
        <g class="node ${node.type} ${selected ? "is-selected" : ""} ${node.focal ? "focal" : ""} ${dimmed ? "is-dimmed" : ""}" data-node-id="${escapeHtml(node.id)}" transform="translate(${node.x} ${node.y})" role="button" tabindex="0" aria-label="${escapeHtml(`?ãÂ? ${node.title}`)}">
...
function updateModeUI(sceneNodes) {
  ...
  if (uiState.isFocus && node) {
    renderBreadcrumb([
      { label: "Á∏ΩË¶Ω", action: () => goToOverview() },
      { label: TAXONOMY_LABELS[node.taxonomy] || node.taxonomy, action: () => goToOverviewAndCenterTaxonomy(node.taxonomy) },
      { label: node.title },
    ]);
    els.graphHint.textContent = "‰∏≠Â?ÁØÄÈªûÂõ∫ÂÆöÔ??ßÂ??™Á??àÂ??áÊï∏Â≠∏ÊîØ?êÔ?Â§ñÂ??™Á??®Â??ÅÈ?Ë≠â„ÄÅÈ?Ê∏¨Ë??âÁî®ÔºåÁÅ∞?≤Ë??äÂ??æÂº±?úËÅØ??;
  } else {
    renderBreadcrumb([
      { label: "Á∏ΩË¶Ω", action: () => goToOverview() },
      { label: "taxonomy domains" },
    ]);
    els.graphHint.textContent = "?ñÊõ≥Âπ≥Áßª?ÅÊªæËº™Á∏Æ?æÔ?‰∏≠Â§Æ?πÁ?ÈªûÈÄ?é•?ÑÈ??üÔ??æÂ§ßÂæåÊ?‰æùÂ?È°ØÁ§∫?¥Á¥∞?ÑÁ?ÈªûÂ?Á®±Ë??ú‰???;
  }
}
...
function renderDetail() {
  ...
  if (!node) {
    ...
    els.detailPath.textContent = "Â∞öÊú™?∏Â?ÁØÄÈª?;
  ...
  if (path) {
    ...
    els.detailPath.textContent = "Ê≤íÊ?‰æÜÊ?Ë∑ØÂ?";
    els.detailPath.removeAttribute("href");
  }
...
function renderSearchResults() {
  ...
  searchSection.innerHTML = buildSearchResultsHtml(state.query, matches, {
    escapeHtml,
    messages: {
      emptyPrefix: "?????ÅÊ≥µ??Ñ¬Ä?",
      emptySuffix: "????≠¬Ä?∫Óµ¢??",
      resultCountPrefix: "?Ó∞??ØÓ????",
      resultCountSuffix: "??",
    },
  });
  return;
  if (!matches.length) {
    searchSection.innerHTML = `<p style="color:var(--muted);font-size:0.88rem;">?æ‰??∞Á¨¶?à„Ä?{escapeHtml(state.query)}?çÁ?ÁØÄÈªû„Ä?/p>`;
    return;
  }
  let html = `<div class="section-head"><h3>?úÂ?ÁµêÊ?Ôº?{matches.length}Ôº?/h3></div><div class="pill-list">`;
