...
CHINESE_NAME_RE = re.compile(r"[\u4e00-\u9fff]{2,4}[（(]?\d{4}")
DIRECT_NEGATION_RE = re.compile(r"不是.{0,20}而是")
...
HISTORY_SECTION_BY_TYPE = {
    "concept": "歷史背景",
    "law": "歷史背景",
    "quantity": "歷史背景",
    "experiment": "歷史背景",
    "mathematical_tool": "歷史背景",
}

DERIVATION_REQUIRED_TYPES = {"law", "mathematical_tool"}
DERIVATION_OPTIONAL_TYPES = {"concept"}
GROUPED_RELATED_LINK_TYPES = {"concept", "law", "experiment"}
SYMBOL_SECTION_AUDIT_TYPES = {"concept", "law", "quantity"}
SYMBOL_SECTION_NAME = "符號與單位"
RELATED_LINKS_SECTION_NAME = "相關連結"
DERIVATION_SECTION_NAME = "推導"
...
def has_history_anchor(text: str) -> bool:
    return bool(YEAR_RE.search(text) or LATIN_NAME_RE.search(text) or CHINESE_NAME_RE.search(text))
...
def audit_note(path: Path, vault: Path, schema: dict) -> list[tuple[str, str]]:
    ...
    if DIRECT_NEGATION_RE.search(strip_markdown(body)):
        issues.append(("banned_pattern", f"{rel_path}: contains discouraged '不是A而是B' pattern"))

    meaning_section = MEANING_SECTION_BY_TYPE.get(note_type)
    if meaning_section and meaning_section in sections:
        min_chars = int(thresholds.get("meaning_min_chars", 100))
    ...
    derivation_section = sections.get(DERIVATION_SECTION_NAME)
    derivation_required = note_type in DERIVATION_REQUIRED_TYPES or (
        note_type in DERIVATION_OPTIONAL_TYPES and DERIVATION_SECTION_NAME in sections
    )
    if derivation_required and derivation_section is not None:
        min_chars = int(thresholds.get("derivation_min_chars", 80))
        ...
        if bool(thresholds.get("require_latex_in_derivation", True)) and not has_formula(derivation_section):
            issues.append(("derivation_missing_formula", f"{rel_path}: {DERIVATION_SECTION_NAME} has no LaTeX formula"))

    history_section_name = HISTORY_SECTION_BY_TYPE.get(note_type)
    if history_section_name and history_section_name in sections:
    ...
    if (
        note_type in SYMBOL_SECTION_AUDIT_TYPES
        and bool(thresholds.get("require_symbol_section_if_formula_present", True))
        and has_formula(body)
    ):
    ...
    if note_type in GROUPED_RELATED_LINK_TYPES and bool(thresholds.get("require_grouped_related_links", True)):
        related_text = sections.get(RELATED_LINKS_SECTION_NAME, "")
        if related_text and not related_links_grouped(related_text):
